...
var PLAN_FEATURES = [
  ...
  { label: { en: 'Intro video', ar: 'فيديو تعريفي' }, val: function (tr) { if (!tr.video) return false; var m = Math.round((tr.limits.videoMaxSec || 0) / 60); return LANG === 'ar' ? (m + ' دقيقة') : (m + ' min'); } },
  ...
  { label: { en: 'Visit & view analytics', ar: 'إحصائيات الزيارات والمشاهدات' }, val: function (tr) { return tr.stats === 'none' ? false : tr.stats === 'advanced' ? (LANG === 'ar' ? 'متقدمة' : 'Advanced') : (LANG === 'ar' ? 'احترافية' : 'Professional'); } },
